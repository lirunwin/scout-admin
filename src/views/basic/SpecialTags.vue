...
export default {
  ...
  computed: {
    ...mapGetters(['specialTags']),
    availableSpecialTags() {
      return this.specialTags.filter(tag => tag.status !== this.status.deprecated.value);
    },
    constant() {
    ...
    tableHeaders() {
      return this.constant.specialTagTableHeaders
    }
  },
  methods: {
    ...mapActions([
      'getSpecialTags',
      'addOrUpdataSpecialTag',
      'updataSpecialTagStatus',
      'resetSpecialTags'
    ]),
    newTag() {
    ...
    saveTag(tag = this.tag) {
      ...
      this.addOrUpdataSpecialTag(tag)
        .then(() => {
          this.closeDialog();
    ...
    toggleSelectAll() {
      ...
      else this.selectedTags = this.specialTags.slice()
    },
    search() {
