        ...
        <v-card-text class="py-0">
          <v-container grid-list-md
            ...
            <v-layout row
              ...
              <v-flex xs12
                class="py-0">
                <v-select :items="forwardLinksTypes"
                  v-model="tag.linktype"
                  item-text="label"
                  item-value="value"
                  label="类型"></v-select>
              </v-flex>
              <v-flex xs12
                ...
                <v-text-field name="name"
                  label="标题"
                  v-model="tag.title"></v-text-field>
              </v-flex>
              <v-flex xs12
                class="py-0">
                <v-text-field name="name"
                  label="跳转链接"
                  v-model="tag.jumpurl"></v-text-field>
              </v-flex>
              <v-flex xs12>
                <ImageUploader class="px-2" v-model="tag.imgpath"></ImageUploader>
              </v-flex>
              <v-flex xs12
                class="py-0">
                <v-text-field name="name"
...
export default {
  ...
  computed: {
    ...mapGetters(['forwardLinks']),
    availableForwardLinks() {
      return this.forwardLinks.filter(tag => tag.status !== this.status.deprecated.value);
    },
    constant() {
    ...
    tableHeaders() {
      return this.constant.forwardLinkTableHeaders
    }
  },
  methods: {
    ...mapActions([
      'getForwardLinks',
      'addOrUpdataForwardLink',
      'updataForwardLinkStatus',
      'resetForwardLinks'
    ]),
    newTag() {
    ...
    saveTag(tag = this.tag) {
      ...
      this.addOrUpdataForwardLink(tag)
        .then(() => {
          this.closeDialog();
    ...
    toggleSelectAll() {
      ...
      else this.selectedTags = this.forwardLinks.slice()
    },
    search() {
